<?php 
$page_title = "Summer 2023 Courses";
$page_description = "Manhattan High School Summer 2023 Courses";

$path = $_SERVER['DOCUMENT_ROOT'];
$path .= "/includes/header.php";
	include_once($path);
?>

	<main id="directory">
		<ul class="contenthead"><li><a href="/pastcourses.php"><h2>Past Courses</h2></li></a><li class="lastcontenthead"><h2>Summer 2023 Courses</h2></li><li class="rightcontenthead"><span class="rightlink"><button class="btn btn-warning excelexport btn-md" type="button" title="Excel Export"></button></span></li></ul>
		<table id="directorylist" class="directorylist" data-paging="true" data-filtering="true" data-filter-position="center" data-sorting="true" data-paging-limit="4" data-filter-placeholder="Search (options on the right)" data-exporting="true">
			<thead>
			<tr class="header">
				<th>Title (PDF links for syllabi)</th>
				<th data-breakpoints="xs sm md">Department</th>
				<th>Teacher</th>
				<th data-breakpoints="xs sm">Room</th>
				<th data-breakpoints="xs">Period/Band</th>
				<th data-breakpoints="xs sm md">Site</th>
			</tr>
			</thead>
			<tbody>
				<tr>
					<td>Algebra I 1/4</td>
					<td>Mathematics</td>
					<td>Heath</td>
					<td>222</td>
					<td>BC, DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Algebra I 1/4</td>
					<td>Mathematics</td>
					<td>Khanum</td>
					<td>308</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Algebra I 2/4</td>
					<td>Mathematics</td>
					<td>Heath</td>
					<td>222</td>
					<td>BC, DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Algebra I 2/4</td>
					<td>Mathematics</td>
					<td>Khanum</td>
					<td>308</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Algebra I 3/4</td>
					<td>Mathematics</td>
					<td>Jin</td>
					<td>216</td>
					<td>DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Algebra I 3/4</td>
					<td>Mathematics</td>
					<td>Khanum</td>
					<td>308</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Algebra I 4/4</td>
					<td>Mathematics</td>
					<td>Jin</td>
					<td>216</td>
					<td>DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Algebra I 4/4</td>
					<td>Mathematics</td>
					<td>Khanum</td>
					<td>308</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Career Education</td>
					<td>Careers</td>
					<td>Afzal</td>
					<td>402</td>
					<td>5</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Career Education</td>
					<td>Careers</td>
					<td>Gonzalez</td>
					<td>104</td>
					<td>5</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Career Education</td>
					<td>Careers</td>
					<td>Khanum</td>
					<td>308</td>
					<td>6</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Career Education</td>
					<td>Careers</td>
					<td>Verde</td>
					<td>317</td>
					<td>6</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Career Education</td>
					<td>Careers</td>
					<td>Watkins</td>
					<td>408</td>
					<td>6</td>
					<td>Main</td>
				</tr>
				<tr>
					<td><a href="/syllabi/2022_7/Earth_Science_1_2_Syllabus_Tahir.pdf" target="_blank">Earth Science 1/2</a></td>
					<td>Physical Science</td>
					<td>Tahir</td>
					<td>212</td>
					<td>DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Earth Science 1/2</td>
					<td>Physical Science</td>
					<td>Watkins</td>
					<td>408</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Earth Science 2/2</td>
					<td>Physical Science</td>
					<td>Tahir</td>
					<td>212</td>
					<td>DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Earth Science 2/2</td>
					<td>Physical Science</td>
					<td>Watkins</td>
					<td>408</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>English 1/8</td>
					<td>Core English</td>
					<td>Bell</td>
					<td>218</td>
					<td>DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>English 1/8</td>
					<td>Core English</td>
					<td>Verde</td>
					<td>317</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>English 2/8</td>
					<td>Core English</td>
					<td>Bell</td>
					<td>218</td>
					<td>DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>English 2/8</td>
					<td>Core English</td>
					<td>Verde</td>
					<td>317</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>English 3/8</td>
					<td>Core English</td>
					<td>Bell</td>
					<td>218</td>
					<td>BC</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>English 3/8</td>
					<td>Core English</td>
					<td>Bennett</td>
					<td>205</td>
					<td>DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>English 3/8</td>
					<td>Core English</td>
					<td>Verde</td>
					<td>317</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>English 4/8</td>
					<td>Core English</td>
					<td>Bell</td>
					<td>218</td>
					<td>BC</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>English 4/8</td>
					<td>Core English</td>
					<td>Bennett</td>
					<td>205</td>
					<td>DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>English 4/8</td>
					<td>Core English</td>
					<td>Verde</td>
					<td>317</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>English 5/8</td>
					<td>Core English</td>
					<td>Bennett</td>
					<td>205</td>
					<td>BC</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>English 5/8</td>
					<td>Core English</td>
					<td>Kennedy</td>
					<td>210</td>
					<td>DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>English 5/8</td>
					<td>Core English</td>
					<td>Verde</td>
					<td>317</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>English 6/8</td>
					<td>Core English</td>
					<td>Bennett</td>
					<td>205</td>
					<td>BC</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>English 6/8</td>
					<td>Core English</td>
					<td>Kennedy</td>
					<td>210</td>
					<td>DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>English 6/8</td>
					<td>Core English</td>
					<td>Verde</td>
					<td>317</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>English 7/8</td>
					<td>Core English</td>
					<td>Kennedy</td>
					<td>210</td>
					<td>DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>English 7/8</td>
					<td>Core English</td>
					<td>Verde</td>
					<td>317</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>English 8/8</td>
					<td>Core English</td>
					<td>Kennedy</td>
					<td>210</td>
					<td>DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>English 8/8</td>
					<td>Core English</td>
					<td>Verde</td>
					<td>317</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Financial Literacy</td>
					<td>Business Elective</td>
					<td>Gonzalez</td>
					<td>104</td>
					<td>12, 34</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Geometry 1/2</td>
					<td>Mathematics</td>
					<td>Jin</td>
					<td>216</td>
					<td>DE</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Geometry 1/2</td>
					<td>Mathematics</td>
					<td>Khanum</td>
					<td>308</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Geometry 2/2</td>
					<td>Mathematics</td>
					<td>Jin</td>
					<td>216</td>
					<td>DE</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Geometry 2/2</td>
					<td>Mathematics</td>
					<td>Khanum</td>
					<td>308</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Global 1/4</td>
					<td>Social Studies</td>
					<td>Afzal</td>
					<td>402</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Global 1/4</td>
					<td>Social Studies</td>
					<td>Curry</td>
					<td>208</td>
					<td>BC, DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Global 2/4</td>
					<td>Social Studies</td>
					<td>Afzal</td>
					<td>402</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Global 2/4</td>
					<td>Social Studies</td>
					<td>Curry</td>
					<td>208</td>
					<td>BC, DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Global 3/4</td>
					<td>Social Studies</td>
					<td>Afzal</td>
					<td>402</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Global 3/4</td>
					<td>Social Studies</td>
					<td>Turner</td>
					<td>206</td>
					<td>BC, DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Global 4/4</td>
					<td>Social Studies</td>
					<td>Afzal</td>
					<td>402</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Global 4/4</td>
					<td>Social Studies</td>
					<td>Turner</td>
					<td>206</td>
					<td>BC, DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Government &amp; Economics 1/2</td>
					<td>Social Studies</td>
					<td>Afzal</td>
					<td>402</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Government &amp; Economics 1/2</td>
					<td>Social Studies</td>
					<td>Kennedy</td>
					<td>210</td>
					<td>BC</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Government &amp; Economics 2/2</td>
					<td>Social Studies</td>
					<td>Afzal</td>
					<td>402</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Government &amp; Economics 2/2</td>
					<td>Social Studies</td>
					<td>Kennedy</td>
					<td>210</td>
					<td>BC</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Health</td>
					<td>PE and Health</td>
					<td>Geller</td>
					<td>226</td>
					<td>BC</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Health</td>
					<td>PE and Health</td>
					<td>Waver</td>
					<td>315</td>
					<td>34</td>
					<td>Main</td>
				</tr>
				<tr>
					<td><a href="/syllabi/2022_7/Living_Environment_1_4_Syllabus_Rojas.pdf" target="_blank">Living Environment 1/4</a></td>
					<td>Life Science</td>
					<td>Rojas</td>
					<td>220</td>
					<td>BC, DE</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Living Environment 1/4</td>
					<td>Life Science</td>
					<td>Watkins</td>
					<td>408</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td><a href="/syllabi/2022_7/Living_Environment_2_4_Syllabus_Rojas.pdf" target="_blank">Living Environment 2/4</a></td>
					<td>Life Science</td>
					<td>Rojas</td>
					<td>220</td>
					<td>BC, DE</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Living Environment 2/4</td>
					<td>Life Science</td>
					<td>Watkins</td>
					<td>408</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td><a href="/syllabi/2022_7/Living_Environment_3_4_Syllabus_Rojas.pdf" target="_blank">Living Environment 3/4</a></td>
					<td>Life Science</td>
					<td>Rojas</td>
					<td>220</td>
					<td>GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td><a href="/syllabi/2022_7/Living_Environment_3_4_Syllabus_Tahir.pdf" target="_blank">Living Environment 3/4</a></td>
					<td>Life Science</td>
					<td>Tahir</td>
					<td>212</td>
					<td>BC</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Living Environment 3/4</td>
					<td>Life Science</td>
					<td>Watkins</td>
					<td>408</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td><a href="/syllabi/2022_7/Living_Environment_4_4_Syllabus_Rojas.pdf" target="_blank">Living Environment 4/4</a></td>
					<td>Life Science</td>
					<td>Rojas</td>
					<td>220</td>
					<td>GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Living Environment 4/4</td>
					<td>Life Science</td>
					<td>Tahir</td>
					<td>212</td>
					<td>BC</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Living Environment 4/4</td>
					<td>Life Science</td>
					<td>Watkins</td>
					<td>408</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Physical Education</td>
					<td>PE and Health</td>
					<td>Geller</td>
					<td>226</td>
					<td>D, E, F</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Physical Education</td>
					<td>PE and Health</td>
					<td>Geller</td>
					<td>702</td>
					<td>A</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Physical Education</td>
					<td>PE and Health</td>
					<td>Waver</td>
					<td>GYM</td>
					<td>5, 6</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Spanish 1/2</td>
					<td>Foreign Language</td>
					<td>Gonzalez</td>
					<td>104</td>
					<td>78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Spanish 2/2</td>
					<td>Foreign Language</td>
					<td>Gonzalez</td>
					<td>104</td>
					<td>78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>Spanish 1/6</td>
					<td>Foreign Language</td>
					<td>Velez</td>
					<td>204</td>
					<td>BC, DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Spanish 2/6</td>
					<td>Foreign Language</td>
					<td>Velez</td>
					<td>204</td>
					<td>BC, DE, GH</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>Studio Art</td>
					<td>Arts</td>
					<td>Collins</td>
					<td>211</td>
					<td>A, B, C, D, E, F</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>US History 1/2</td>
					<td>Social Studies</td>
					<td>Afzal</td>
					<td>402</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>US History 1/2</td>
					<td>Social Studies</td>
					<td>Prisco</td>
					<td>214</td>
					<td>BC, DE</td>
					<td>Beacon</td>
				</tr>
				<tr>
					<td>US History 2/2</td>
					<td>Social Studies</td>
					<td>Afzal</td>
					<td>402</td>
					<td>12, 34, 78</td>
					<td>Main</td>
				</tr>
				<tr>
					<td>US History 2/2</td>
					<td>Social Studies</td>
					<td>Prisco</td>
					<td>214</td>
					<td>BC, DE</td>
					<td>Beacon</td>
				</tr>
			</tbody>
		</table>
		<div id="rowcount">
			<ul>
				<li>Rows to display:</li>
				<li><button type="button" data-page-size="10">10</button></li>
				<li><button type="button" data-page-size="20">20</button></li>
				<li><button type="button" data-page-size="50">50</button></li>
				<li><button type="button" data-page-size="100">100</button></li>
				<li><button type="button" data-page-size="200">200</button></li>
			</ul>
		</div>
	</main>

<script>
function myFunction() {
  var input, filter, table, tr, td, i;
  input = document.getElementById("directorysearch");
  filter = input.value.toUpperCase();
  table = document.getElementById("directorylist");
  tr = table.getElementsByTagName("tr");
  for (i = 1; i < tr.length; i++) {
        var tds = tr[i].getElementsByTagName("td");
        var found = false;
        for (ii = 0; ii < tds.length && !found; ii++) {
            if (tds[ii].textContent.toUpperCase().indexOf(filter) > -1) {
                found = true;
                break;
            }
        }
        tr[i].style.display = found?"":"none";
    }
}
</script>
	
<script>jQuery(function($){
	$('.directorylist').footable();
});</script>
	
<script>
$('[data-page-size]').on('click', function(e){
	e.preventDefault();
	var newSize = $(this).data('pageSize');
	FooTable.get('#directorylist').pageSize(newSize);
});</script>
	
<script>
$('.excelexport').on('click', function (e) {
    var csv = FooTable.get('#directorylist').toCSV(true);
    /*  console.log ("csv: " + csv);*/
    var hiddenElement = document.createElement('a');
    hiddenElement.href = 'data:application/csv;charset=utf-8,' + encodeURI(csv);
    hiddenElement.target = '_blank';
    hiddenElement.download = 'Courses_2020_7.csv';
    hiddenElement.click();
}); 
</script>

<?php
$path = $_SERVER['DOCUMENT_ROOT'];
$path .= "/includes/footer.php";
	include_once($path);
?>